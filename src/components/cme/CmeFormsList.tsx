...
export function CmeFormsList() {
  ...
  useEffect(() => { fetch("/api/cme/formularios").then(async (response) => { const payload = await response.json(); if (!response.ok) throw new Error(payload.error); setForms(payload.formularios); }).catch((cause) => setError(cause instanceof Error ? cause.message : "Não foi possível carregar os checklists CME.")).finally(() => setLoading(false)); }, []);
  ...
  return <>
    <div className="card" style={{ marginBottom: 16 }}><div className="grid grid-3">
      <div className="field"><label>Buscar</label><div style={{ position: "relative" }}><Search aria-hidden="true" size={17} style={{ left: 10, position: "absolute", top: 11 }} /><input className="input" style={{ paddingLeft: 34 }} value={query} onChange={(e) => { setQuery(e.target.value); setPage(1); }} placeholder="Responsável ou setor..." /></div></div>
...
export function CmeListHeader() { return <Link className="button" href="/cme/new"><Plus size={18} />Novo checklist</Link>; }
