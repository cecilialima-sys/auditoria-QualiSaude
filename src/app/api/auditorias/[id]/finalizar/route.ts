    ...
    const result = await service.finalizeAndGenerate(
      {
        ...
        responses: details.checklist.perguntas.map((question) => {
          ...
          return {
            ...
            // O relatório original é o registro fiel da auditoria. A redação com IA
            // permanece exclusivamente no relatório técnico independente.
            evidence: answer?.evidenciaOriginal || answer?.evidencia,
            risk: answer?.risco
          };
        })
      },
