...
import { cmeChecklistClassifications, cmeChecklistItems, cmeEquipments, cmeIndicators, cmeNonConformities, cmeProcesses, type CmeChecklistResponse, type CmeFormInput, type CmeFormRecord } from "@/lib/cme/cmeForm";
export { cmeChecklistClassifications, cmeChecklistItems, cmeEquipments, cmeIndicators, cmeNonConformities, cmeProcesses, type CmeChecklistResponse, type CmeFormInput, type CmeFormRecord } from "@/lib/cme/cmeForm";
...
function validate(input: CmeFormInput): Omit<CmeFormRecord, "id" | "createdAt" | "updatedAt" | "status"> {
  ...
  const validQuestionIds = new Set(cmeChecklistItems.map((item) => item.id));
  const checklistResponses = Array.isArray(input.checklistResponses) ? input.checklistResponses : [];
  const responseIds = new Set<string>();
  const validResponses: CmeChecklistResponse[] = checklistResponses.map((response) => {
    if (!response || !validQuestionIds.has(response.questionId) || responseIds.has(response.questionId)) throw new Error("Resposta de checklist inválida.");
    if (!cmeChecklistClassifications.includes(response.classification)) throw new Error("Classificação do checklist inválida.");
    responseIds.add(response.questionId);
    return { questionId: response.questionId, classification: response.classification, evidence: optional(response.evidence) };
  });
  return { ...input, recordDate: input.recordDate!, recordTime: input.recordTime!, responsible: input.responsible!.trim(), sector: input.sector!.trim(), unit: input.unit!.trim(), recordType: input.recordType!.trim(), process: input.process!, equipment: input.equipment!, equipmentOtherDescription: optional(input.equipmentOtherDescription), materialName: input.materialName!.trim(), materialCode: optional(input.materialCode), quantity, lot: optional(input.lot), materialNotes: optional(input.materialNotes), chemicalIndicator: input.chemicalIndicator!, biologicalIndicator: input.biologicalIndicator!, bowieDickTest: input.bowieDickTest!, nonConformities, nonConformityOther: optional(input.nonConformityOther), generalNotes: optional(input.generalNotes), signerName: input.signerName!.trim(), signerRole: input.signerRole!.trim(), signatureDate: input.signatureDate!, signatureTime: input.signatureTime!, checklistResponses: validResponses };
}

function readLocal(): CmeFormRecord[] { if (!state.cmeForms) { try { state.cmeForms = existsSync(storePath) ? JSON.parse(readFileSync(storePath, "utf8")) : []; } catch { state.cmeForms = []; } } return state.cmeForms ?? []; }
function saveLocal() { mkdirSync(dirname(storePath), { recursive: true }); writeFileSync(storePath, JSON.stringify(readLocal(), null, 2)); }
function serialize(row: any): CmeFormRecord { return { id: row.id, recordDate: new Date(row.recordDate).toISOString().slice(0, 10), recordTime: row.recordTime, responsible: row.responsible, sector: row.sector, unit: row.unit, recordType: row.recordType, process: row.process, equipment: row.equipment, equipmentOtherDescription: row.equipmentOtherDescription ?? undefined, materialName: row.materialName, materialCode: row.materialCode ?? undefined, quantity: row.quantity, lot: row.lot ?? undefined, materialNotes: row.materialNotes ?? undefined, chemicalIndicator: row.chemicalIndicator, biologicalIndicator: row.biologicalIndicator, bowieDickTest: row.bowieDickTest, nonConformities: Array.isArray(row.nonConformities) ? row.nonConformities : [], nonConformityOther: row.nonConformityOther ?? undefined, generalNotes: row.generalNotes ?? undefined, signerName: row.signerName, signerRole: row.signerRole, signatureDate: new Date(row.signatureDate).toISOString().slice(0, 10), signatureTime: row.signatureTime, checklistResponses: Array.isArray(row.checklistResponses) ? row.checklistResponses : [], createdAt: new Date(row.createdAt).toISOString(), updatedAt: new Date(row.updatedAt).toISOString(), status: "Registrado" }; }
async function prisma<T>(operation: (client: ReturnType<typeof getPrismaClient>) => Promise<T>): Promise<T | null> { if (state.cmePrismaUnavailable) return null; try { return await operation(getPrismaClient()); } catch (error) { state.cmePrismaUnavailable = true; console.error("[cme] Prisma indisponível; usando armazenamento local.", error); return null; } }

export async function listCmeForms() { const rows = await prisma<unknown[]>((client) => (client as any).cmeForm.findMany({ orderBy: { recordDate: "desc" } })); return rows ? rows.map(serialize) : readLocal().slice().sort((a, b) => b.recordDate.localeCompare(a.recordDate)); }
...
export async function deleteCmeForm(id: string) { const result = await prisma<{ count: number }>((client) => (client as any).cmeForm.deleteMany({ where: { id } })); if (result) return result.count === 1; const index = readLocal().findIndex((item) => item.id === id); if (index < 0) return false; readLocal().splice(index, 1); saveLocal(); return true; }
