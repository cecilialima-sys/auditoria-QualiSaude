...
function row(item: TechnicalReportItem) {
  const references = Array.isArray(item.normativeReferences) && item.normativeReferences.length
    ...
    : "";
  return `<tr><td class="number">${text(item.number)}</td><td class="requirement">${text(item.requirement)}</td><td class="mark">${item.classification === "Conforme" ? "X" : ""}</td><td class="mark">${item.classification === "Não conforme" ? "X" : ""}</td><td class="mark">${item.classification === "Não se aplica" ? "X" : ""}</td><td class="finding">${finding(item)}${references}</td></tr>`;
}
